...
input_file = "%s/update_acc_%s"%(in_dir,date)
...
def  extract_info(filename,version,Nucleotide_ID_num,paper_IN_num,GI):
    genome_gff = []
...
def main():
    ...
    for eachline in file1:
        ...
        line = eachline.strip()
        version = line
        print ("正在下载第%s个数据，version:%s"%(count_number,version))
        #filename = "%s/Coronaviridae_genbank_%s/%s.gbk"%(dir,date,locus)
        filename = "/home/sxq/NCBI/data/download_data/mysql/nucleotide/Coronaviridae_genbank/%s.gbk"%(version)
        if os.path.isfile(filename):
            gi_handle = Entrez.esummary(db="nucleotide", id=version)
            record = Entrez.read(gi_handle)
            GI = record[0]["Id"]
            extract_info(filename,version,Nucleotide_ID_num,paper_IN_num,GI)
        else:
            gi_handle = Entrez.esummary(db="nucleotide", id=version)
            record = Entrez.read(gi_handle)
            GI = record[0]["Id"]
